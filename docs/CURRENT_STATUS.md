...
- Current boundaries: MobHealthBar can support health/name danger readability, but threat-tier labels are policy-only. Traveler's Titles biome/dimension titles are live, but Atlas coordinate-region titles are policy-only until a runtime hook exists.
...
- Next step after terrain/water acceptance: in-game visual pass for dropped high-rarity loot beams, tooltip order, title pack order, and MobHealthBar readability. Do not redesign the menu or add UI mods from this scaffold.
