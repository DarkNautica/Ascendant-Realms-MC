param(
    ...
    [string]$MinecraftInstance = "C:\Users\Jayden\curseforge\minecraft\Instances\Ascendant Realms (2)",
    [string]$OutputJar = "mods\ascendant-atlas-regions-0.1.0.jar"
)
...
$ModRoot = Join-Path $RepoRoot "local-mods\ascendant-atlas-regions"
...
if (-not $ResolvedOutputJar.StartsWith($RepoRoot, [System.StringComparison]::OrdinalIgnoreCase)) {
    throw "Refusing to write helper jar outside the repo: $ResolvedOutputJar"
}
...
if (-not $javac) {
    throw "javac is required. Install or select a Java 17 JDK before building the Atlas helper."
}
...
$instanceModJars = @()
$instanceModsPath = Join-Path $MinecraftInstance "mods"
if (Test-Path -LiteralPath $instanceModsPath) {
    $instanceModJars = @(Get-ChildItem -LiteralPath $instanceModsPath -Filter "immersive-portals-*.jar" |
        Where-Object { $_.Length -gt 0 -and (Test-ZipJar -Path $_.FullName) } |
        ForEach-Object { $_.FullName })
}

if (-not $instanceModJars -or $instanceModJars.Count -eq 0) {
    throw "Immersive Portals jar was not found under $instanceModsPath. Sync the active instance before rebuilding the Atlas helper."
}
...
$classpath = (($libraryJars + $instanceModJars) -join ";")
...
if ($LASTEXITCODE -ne 0) {
    throw "Atlas helper javac build failed."
}

$manifestPath = Join-Path $BuildRoot "MANIFEST.MF"
@(
    "Manifest-Version: 1.0",
    "Specification-Title: Ascendant Atlas Regions",
    "Specification-Version: 0.1.0",
    "Implementation-Title: Ascendant Atlas Regions",
...
if ($LASTEXITCODE -ne 0) {
    throw "Atlas helper jar packaging failed."
}
